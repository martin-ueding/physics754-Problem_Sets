...
I have to show that
\[
    \eval{\dod{}\tau S_\text{gr}(f^*_\tau g)}_{\tau=0} = \frac{1}{8\piup G}
    \intop_{\R^4} \dif{^4 x} \, \sqrt{|g|} G^{\mu\nu} X_{(\mu;\nu)}.
\]

The various forks induced by the product rule will make this calculation
nonlinear, I will try to perform an in-order traversal of all the steps. First
I will put the transformed $g$ into the definition of the action.
\[
